...
- [ ] Task 1: Create `SearchIndexer.swift` in `DictlyKit/Sources/DictlyStorage/` (AC: #1, #2, #3)
  - [ ] 1.1 Create `SearchIndexer.swift` — public class with `import CoreSpotlight` and `import UniformTypeIdentifiers`
  - [ ] 1.2 Define domain identifier constant: `"com.dictly.tags"` for batch operations
  - [ ] 1.3 Implement `indexTag(_ tag: Tag)` — creates `CSSearchableItem` from Tag properties using `CSSearchableIndex.default()`
  - [ ] 1.4 Build `CSSearchableItemAttributeSet(contentType: .text)` with:
    - `title` = tag label
  ...
  - [ ] 1.5 Use `tag.uuid.uuidString` as `uniqueIdentifier` for the `CSSearchableItem`
  - [ ] 1.6 Implement `updateTag(_ tag: Tag)` — re-indexes the tag (same as `indexTag` since re-indexing with same `uniqueIdentifier` replaces the entry)
  - [ ] 1.7 Implement `removeTag(id: UUID)` — calls `deleteSearchableItems(withIdentifiers: [id.uuidString])`
  - [ ] 1.8 Implement `removeAllTagsForSession(sessionID: UUID)` — deletes by domain identifier or iterates tag UUIDs
  - [ ] 1.9 Implement `removeAllItems()` — calls `deleteAllSearchableItems()` for cleanup/reset
  - [ ] 1.10 All public methods use `async throws` — Core Spotlight operations are async
  - [ ] 1.11 Add `DictlyError.search(SearchError)` case to `DictlyError.swift` with `.indexingFailed(String)` and `.deletionFailed(String)` sub-cases

- [ ] Task 2: Integrate indexing into tag creation flow — iOS (AC: #1)
  - [ ] 2.1 In `DictlyiOS/Tagging/TaggingService.swift`, after `modelContext.insert(tag)` succeeds, call `SearchIndexer().indexTag(tag)` in a `.task` or `Task { }`
  - [ ] 2.2 Indexing failure must NOT block tag creation — catch errors and log via `os.Logger(category: "search")`
  - [ ] 2.3 Verify tag's session relationship is established before indexing (needed for session title/campaign keywords)

- [ ] Task 3: Integrate indexing into Mac import flow (AC: #1)
  - [ ] 3.1 In `DictlyMac/Import/ImportService.swift`, after `modelContext.save()` in `performImport`, batch-index all imported tags
  - [ ] 3.2 Use `CSSearchableIndex.default().indexSearchableItems(items)` for batch efficiency — build all `CSSearchableItem` objects first, then index in one call
  - [ ] 3.3 Import failure must NOT be caused by indexing failure — catch and log; import succeeds regardless

- [ ] Task 4: Integrate indexing into transcription completion and edit flows (AC: #2)
  - [ ] 4.1 In `DictlyMac/Transcription/TranscriptionEngine.swift`, after transcription text is written to `tag.transcription`, call `SearchIndexer().updateTag(tag)`
  - [ ] 4.2 In `DictlyMac/Review/TagDetailPanel.swift`, in the `commitTranscription` logic (around line 66), call `SearchIndexer().updateTag(tag)` after saving edited transcription
  - [ ] 4.3 In `DictlyMac/Review/TagDetailPanel.swift`, also call `updateTag` after tag notes are saved (notes are part of searchable content)
  - [ ] 4.4 In `DictlyMac/Review/TagDetailPanel.swift`, also call `updateTag` after tag label rename (label is the title)
  - [ ] 4.5 All update calls are fire-and-forget `Task { }` — never block UI for index updates

- [ ] Task 5: Integrate removal on tag/session deletion (AC: #3)
  - [ ] 5.1 In `DictlyMac/Review/TagDetailPanel.swift`, before or after deleting a tag from SwiftData, call `SearchIndexer().removeTag(id: tag.uuid)`
  - [ ] 5.2 When a session is deleted (if supported in current UI), call `SearchIndexer().removeAllTagsForSession(sessionID:)` or iterate tags and remove each
  - [ ] 5.3 Deletion from index is fire-and-forget — if it fails, items become orphaned but cause no harm (they'll fail to resolve on click)

- [ ] Task 6: Ensure macOS system Spotlight integration (AC: #4)
  - [ ] 6.1 Verify `CSSearchableIndex.default()` is used (not a custom-named index) — this is what surfaces items in system Spotlight
  - [ ] 6.2 Add `CoreSpotlight` framework to both iOS and Mac targets in Xcode project settings (Frameworks, Libraries, and Embedded Content)
  - [ ] 6.3 Add `CoreSpotlight` framework dependency to `DictlyStorage` target in `DictlyKit/Package.swift` — no SPM dependency needed since it's a system framework, but the `import CoreSpotlight` must resolve
  - [ ] 6.4 Test that indexed items appear in macOS Spotlight when searching for a tag label

- [ ] Task 7: Write unit tests (AC: #1–#4)
  - [ ] 7.1 Create `DictlyKit/Tests/DictlyStorageTests/SearchIndexerTests.swift`
  - [ ] 7.2 Test: `testIndexTag_createsSearchableItem` — verify CSSearchableItem is created with correct uniqueIdentifier, title, textContent, keywords
  - [ ] 7.3 Test: `testUpdateTag_replacesExistingItem` — verify re-indexing with same UUID works
  - [ ] 7.4 Test: `testRemoveTag_deletesFromIndex` — verify deletion by identifier
  - [ ] 7.5 Test: `testRemoveAllItems_clearsIndex` — verify full cleanup
  - [ ] 7.6 Test: `testIndexTag_withNilTranscription_indexesLabelOnly` — verify partial data handling
  - [ ] 7.7 Test: `testIndexTag_withAllFields_populatesAllAttributes` — verify label + transcription + notes + category all indexed
  - [ ] 7.8 Ensure all existing DictlyKit tests still pass (0 regressions)
